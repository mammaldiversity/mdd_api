...
/// - Strips whitespace.
/// - Replaces `#` with `No`.
/// - Removes any character that is not alphanumeric, underscore, or space.
/// - Splits on whitespace and case changes to convert to camelCase.
pub fn to_camel(col: &str) -> String {
    let col = col.trim().replace('#', "No");
    
    // Remove non-word/non-space chars
    let cleaned: String = col.chars()
        .filter(|c| c.is_alphanumeric() || *c == '_' || c.is_whitespace())
        .collect();

    // Insert space between lowercase and uppercase: ([a-z])([A-Z]) -> \1 \2
...
pub fn read_file_as_records(path: &Path) -> Result<Vec<HashMap<String, String>>, Box<dyn std::error::Error>> {
    ...
    if ext == "xlsx" || ext == "xls" {
        ...
        let headers: Vec<String> = match rows.next() {
            Some(r) => r.iter().map(|cell| to_camel(&data_to_string(cell))).collect(),
...
fn find_mil_metadata_file(dir: &Path) -> Option<PathBuf> {
...
fn find_mil_image_dir(dir: &Path) -> Option<PathBuf> {
                ...
                for entry in entries.flatten() {
                    ...
                    if p.is_dir() {
                        ...
                        if let Some(ext) = p.extension() {
                            ...
                            if ext_lower == "jpg" || ext_lower == "jpeg" || ext_lower == "png" || ext_lower == "webp" || ext_lower == "tif" || ext_lower == "tiff" {
...
pub fn extract_archive_if_compressed(
    path: &Path,
    temp_dir: &Path,
    ...
    let mil_metadata = find_mil_metadata_file(temp_dir)
        .ok_or_else(|| "Could not find MIL metadata file in the extracted archive".to_string())?;
        
    let mil_img_dir = find_mil_image_dir(temp_dir)
        .ok_or_else(|| "Could not find MIL image directory in the extracted archive".to_string())?;
...
pub fn prepare_metadata(
    mil_file: &Path,
    mdd_file: &Path,
    mil_img_dir: Option<&Path>,
    output_path: &Path,
) -> Result<(), Box<dyn std::error::Error>> {
    let temp_holder = TempDir::new("mil_prep")?;
    let (active_mil_file, active_img_dir) = match extract_archive_if_compressed(mil_file, temp_holder.path())? {
        Some((metadata_path, img_dir_path)) => (metadata_path, img_dir_path),
        None => {
            let img_dir = mil_img_dir.ok_or_else(|| {
                "MIL image directory is required when the MIL file is not a compressed archive."
            })?;
            (mil_file.to_path_buf(), img_dir.to_path_buf())
        }
    };

    println!("Loading MIL metadata from: {:?}", active_mil_file);
    let mil_records = read_file_as_records(&active_mil_file)?;
    println!("Found {} MIL records.", mil_records.len());

    println!("Loading MDD metadata from: {:?}", mdd_file);
    let mdd_records = read_file_as_records(mdd_file)?;
    println!("Found {} MDD records.", mdd_records.len());

    // Build MDD scientificName to ID map
    let mut mdd_map = HashMap::new();
    for rec in mdd_records {
        if let (Some(id_str), Some(genus), Some(epithet)) = (rec.get("id"), rec.get("genus"), rec.get("specificEpithet")) {
            if let Ok(id) = id_str.parse::<u32>() {
                let name = format!("{}_{}", genus, epithet);
                mdd_map.insert(name, id);
            }
        }
    }

    // Scan image directory and build orientation map
    println!("Scanning image directory for dimensions: {:?}", active_img_dir);
    let mut image_map = HashMap::new();
    // Scan recursively for image files
    fn scan_images(dir: &Path, map: &mut HashMap<String, String>) {
                    ...
                    if let Some(ext) = p.extension() {
                        ...
                        if ext_lower == "jpg" || ext_lower == "jpeg" || ext_lower == "png" || ext_lower == "webp" || ext_lower == "tif" || ext_lower == "tiff" {
                            ...
                            match imagesize::size(&p) {
                                ...
                                Err(e) => {
                                    eprintln!("Warning: Failed to read dimensions of image {:?}: {:?}", p, e);
                                }
                            }
                        }
                    }
    ...
    scan_images(&active_img_dir, &mut image_map);
    ...
    for rec in mil_records {
        ...
        let mil_id = match rec.get("milNo") {
            Some(id) => id.clone(),
            None => continue, // Skip rows without milNo
        };
        ...
        let clean_epithet = if is_uncertain {
            raw_epithet.strip_suffix('?').unwrap_or(&raw_epithet).to_string()
        } else {
            raw_epithet.clone()
        };
        ...
        merged_records.push(MilMddRecord {
            mil_id,
            description,
            photographer,
            location,
    ...
    if !missing_image_ids.is_empty() {
        ...
        println!("Missing {} images: {:?}", missing_image_ids.len(), missing_image_ids);
        
        // Filter out records without an orientation
        let valid_records: Vec<MilMddRecord> = merged_records
            .into_iter()
        ...
        println!("Missing images, metadata exported to {:?} with missing image data", output_path);
    } else {
        let json_data = serde_json::to_string(&merged_records)?;
        fs::write(output_path, json_data)?;
        println!("Successfully exported metadata to {:?}", output_path);
    }
...
mod tests {
    ...
    #[test]
    fn test_to_camel() {
        assert_eq!(to_camel("MIL #"), "milNo");
        assert_eq!(to_camel("Genus"), "genus");
        assert_eq!(to_camel("Specific Epithet"), "specificEpithet");
        assert_eq!(to_camel("Description of Image"), "descriptionOfImage");
        assert_eq!(to_camel("Photographer"), "photographer");
        assert_eq!(to_camel("Location Where Image Taken"), "locationWhereImageTaken");
        assert_eq!(to_camel("Date Image Taken"), "dateImageTaken");
        assert_eq!(to_camel("Original File Name"), "originalFileName");
        assert_eq!(to_camel("specificEpithet"), "specificEpithet");
    }
    ...
    fn test_prepare_metadata() {
        ...
        prepare_metadata(&mil_path, &mdd_path, Some(&img_dir), &output_json).unwrap();

        // 5. Read output and verify
        let content = fs::read_to_string(&output_json).unwrap();
        let records: Vec<MilMddRecord> = serde_json::from_str(&content).unwrap();
